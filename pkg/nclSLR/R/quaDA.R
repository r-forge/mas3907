...
#' @param functions logical indicating whether the discriminant functions
#' should be computed explicitly and returned
#' @return An object of class \code{"quada"}, basically a list with the
#' following elements:
#' @return \item{functions}{discriminant functions - a list with an element
#' for each group which, in turn, is a list containing an upper triangular
#' matrix containing the coefficients of the quadratict terms, a vector
#' containing the coefficients of the linear terms and a constant}
...
quaDA <-
  function(variables, group, prior = NULL, validation = NULL,
           learn = NULL, test = NULL, prob = FALSE, functions=FALSE)
  {
    # check inputs
    ...
    if (validation == "none") {
      get_quada = my_quaDA(X, y, 1:n, 1:n, prior, prob, functions)
    ...
    if (validation == "learntest")
    {
      ...
      # apply linDA
      get_quada = my_quaDA(X, y, learn, test, prior, prob, functions)
    ...
    if (validation == "crossval")
    {
      ...
      for (r in 1:10)
      {
        ...
        learn = (1:n)[-test]
        # apply DA
        quada_cv = my_quaDA(X, y, learn, test, prior, prob, FALSE)
        # misclassification error rate
    ...
    ## results
    structure(list(functions = get_quada$functions,
                   confusion = get_quada$conf,
